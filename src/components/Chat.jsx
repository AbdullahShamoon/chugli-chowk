    ...
    return (
        <>
            <div className="component flex flex-col justify-between mx-6 w-full pl-3 relative">
                <div onClick={() => props.setRoom("")} className="back absolute -top-2 right-10 w-14 cursor-pointer"><img src="/images/back.png" alt="" /></div>
                <h3 className='text-2xl font-bold underline'>Room : {props.room}</h3>
                <div className='h-[25rem] overflow-y-auto my-2'>
                    {messages.map((message) => (
                        <div key={message.id}>
                            <span className='font-semibold text-[17px]'>{message.user} : </span>
                ...
                <form onSubmit={handleSubmit} className='flex justify-center items-center gap-2 w-full'>
                    ...
                    <button type='submit'><img src="/images/send.png" width={30} alt="" /></button>
                </form>
            </div>
        </>
    )
